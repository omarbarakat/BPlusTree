...
class BTDriver implements GlobalConst {
	...
	public void runTests() {
		...
		String path = "C:/Users/" + System.getProperty("user.name") + "/DBs";
		new File(path).mkdir();
		dbpath = path + "/BTREE" + random.nextInt() + ".minibase-db";
		logpath = path + "/BTREE" + random.nextInt() + ".minibase-log";
	...
	private void menu() {
		...
		System.out
				.println("[8]   Test2 (new file): insert n records in reverse order");
		System.out
				.println("[9]   Test3 (new file): insert n records in random order");
		System.out
				.println("[10]  Test4 (new file): insert n records in random order");
		...
		System.out.println("\n[16]  Close the file");
		System.out
				.println("[17]  Open which file (input an integer for the file name): ");
		System.out
				.println("[18]  Destroy which file (input an integer for the file name): ");
			...
			try {
				...
				case 0:
					...
					System.out.println(" ***************** The file name is: "
							+ "AAA" + postfix + "  **********");
					file = new BTreeFile("AAA" + postfix, keyType, 100, 0);// naive
																			// delete
					break;
				case 1:
					postfix++;
					...
					System.out.println(" ***************** The file name is: "
							+ "AAA" + postfix + "  **********");
					file = new BTreeFile("AAA" + postfix, keyType, 100, 1);// full
																			// delete
					break;
				case 2:
				...
				case 7:
					...
					keyType = AttrType.attrInteger;
					System.out
							.println("Please input the number of keys to insert: ");
				...
				case 10:
					...
					n = GetStuff.getChoice();
					System.out
							.println("Please input the number of keys to delete: ");
				...
				case 12:
					keyType = AttrType.attrInteger;
					System.out
							.println("Please input the LOWER integer key (null if -3): ");
					lowkeyInt = GetStuff.getChoice();
					lowkey = new IntegerKey(lowkeyInt);
					System.out
							.println("Please input the HIGHER integer key (null if -2): ");
					hikeyInt = GetStuff.getChoice();
				...
				case 13:
					entry = scan.get_next();
					if (entry != null)
						System.out.println("SCAN RESULT: " + entry.key + " "
								+ entry.data);
					else
				...
				case 16:
					file.close();
					System.out
							.println(" ***************** You close the file: "
									+ "AAA" + postfix + "  **********");
					break;
				case 17:
					file.close();
					n = GetStuff.getChoice();
					System.out.println(" ***************** You open the file: "
							+ "AAA" + n + "  **********");
